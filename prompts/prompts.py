import os
from langchain_core.prompts import ChatPromptTemplate
import json
from utils.sawgger_interpreter import reduce_openapi_spec, ReducedOpenAPISpec

class Prompts:
    def __init__(self):
        self.api_ref = self.__create_api_reference(swagger_path=os.environ.get('SWAGGER_PATH', None))
        self.endpoint_desc = self.__create_endpoint_desc(api_ref=self.api_ref)
        self.planner = self.__createChatPromptTemplate("planner")
        self.api_selector = self.__create_api_selector_ChatPromptTemplate(endpoints=self.endpoint_desc)
        self.api_url = self.api_ref.servers[0]['url']
        self.caller = self.__create_caller_ChatPromptTemplate(api_docs='', task='', api_url=self.api_url, background='')
        self.parser = self.__createChatPromptTemplate("parser")
        self.replanner = self.__create_replanner_ChatPromptTemplate()

    def __create_endpoint_desc(self, api_ref:ReducedOpenAPISpec) -> str:
        return self.__format_endpoints(api_ref)

    def __create_api_selector_ChatPromptTemplate(self, endpoints: str) -> ChatPromptTemplate:
        api_selector_prompt = ChatPromptTemplate.from_template(self.__read_prompt(prompt_type="api_selector"))
        api_selector_prompt = ChatPromptTemplate.format_prompt(api_selector_prompt, endpoints=endpoints)
        api_selector_prompt = ChatPromptTemplate.from_messages(
            [
                ("system",
                api_selector_prompt.messages[0].content
                ),
                ("placeholder", "{messages}"),
            ]
        )
        return api_selector_prompt
    
    def __create_caller_ChatPromptTemplate(self, api_docs='', task='', api_url='', background=''):
        caller_prompt = ChatPromptTemplate.from_template(self.__read_prompt(prompt_type="caller"))
        caller_prompt = ChatPromptTemplate.format_prompt(caller_prompt,
                                                        api_docs=api_docs,
                                                        task=task,
                                                        api_url=api_url,
                                                        background=background,
                                                        )
        caller_prompt = ChatPromptTemplate.from_messages(
            [
                (
                    "system",
                    caller_prompt.messages[0].content,
                ),
                ("placeholder", "{messages}"),
            ]
        )
        return caller_prompt

    def __createChatPromptTemplate(self, propmt_type: str):
        return ChatPromptTemplate.from_messages(
            [
                (
                    "system",
                    f"""{self.__read_prompt(prompt_type=propmt_type)}""",
                ),
                ("placeholder", "{messages}"),
            ]
        )
    
    def __create_replanner_ChatPromptTemplate(self):
        replanner_prompt = ChatPromptTemplate.from_template(f"""{self.__read_prompt(prompt_type='replanner')}""")
        return replanner_prompt
    
    def __read_prompt(self, prompt_type: str) -> str:
        options = ["planner", "api_selector", "caller", "replanner", "parser"]
        prompt_type = prompt_type.lower()
        if prompt_type not in options:
            raise ValueError(f"Invalid prompt type: {prompt_type}")
        with open(f"prompts/{prompt_type}_prompt.txt", "r") as f:
            return f.read()


    def __format_endpoints(self, api_ref: ReducedOpenAPISpec) -> str:
        api_name_desc = [f"{endpoint[0]} {endpoint[1].split('.')[0] if endpoint[1] is not None else ''}" for endpoint in
                        api_ref.endpoints]
        api_name_desc = '\n'.join(api_name_desc)
        api_name_desc = api_name_desc.replace('{', '{{').replace('}', '}}')
        return api_name_desc

    def __create_api_reference(self, swagger_path:str) -> ReducedOpenAPISpec:
        if swagger_path is None:
            raise ValueError("SWAGGER_PATH is not set. Please set the SWAGGER_PATH environment variable.")
        if not os.path.exists(swagger_path):
            raise FileNotFoundError(f"Swagger file not found at {swagger_path}.")
        if not swagger_path.endswith('.json'):
            raise ValueError(f"Invalid Swagger file format: {swagger_path}. Expected a .json file.")
        return self.__fetch_api_ref(swagger_path)
    
    def __fetch_api_ref(self, path: str) -> ReducedOpenAPISpec:
        with open(path) as f:
            raw_tmdb_api_spec = json.load(f)
        api_spec = reduce_openapi_spec(raw_tmdb_api_spec, only_required=False)
        return api_spec

    def __change_caller_prompt(self, api_docs=None, task=None, api_url=None, background=None):
        if api_url is None:
            api_url = self.api_ref.servers[0]['url']
        if task is None:
            task = ''
        if background is None:
            background = ''
        if api_docs is None:
            api_docs = ''
        self.caller = self.__create_caller_ChatPromptTemplate(api_docs=api_docs, task=task, api_url=api_url, background=background)

    def updateReplanPrompt(self, main_task: str, original_plan: list[str], past_steps: list[tuple[str, str]]):
        print(past_steps)
        past_steps = [(step[0], str(step[1]).replace("{", "(").replace("}", ")")) for step in past_steps]
        print(past_steps)
        replanner_prompt = ChatPromptTemplate.from_template(f"""{self.__read_prompt(prompt_type='replanner')}""")
        replanner_prompt = ChatPromptTemplate.format_prompt(replanner_prompt,
                                                           input=main_task,
                                                           plan=original_plan,
                                                           past_steps=past_steps)
        replanner_prompt = ChatPromptTemplate.from_messages(
            [
                (
                    "system",
                    replanner_prompt.messages[0].content,
                ),
                ("placeholder", "{messages}"),
            ]
        )
        self.replanner = replanner_prompt