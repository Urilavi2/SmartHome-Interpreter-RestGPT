...
def reduce_openapi_spec(spec: dict, dereference: bool = True, only_required: bool = True, merge_allof: bool = False) -> ReducedOpenAPISpec:
    ...
    endpoints = [(f"{operation_name.upper()} {route}", docs.get("description"), docs) for route, operation in spec["paths"].items() for operation_name, docs in operation.items() if operation_name in ["get", "post", "patch", "delete", "put"]]
